...
#     def __init__(self):
#         # Initialize paths for offload and data storage directories
#         self.offl_folder = "./offload_dir"
#         self.data_folder = "./data"
#         # Use a pre-trained model checkpoint for the summarization task
#         self.checkpoint = "MBZUAI/LaMini-Flan-T5-248M"

#         # Create the necessary directories and load the model and tokenizer
...
#             self.checkpoint, device_map='auto', torch_dtype=torch.float32, offload_folder=self.offl_folder
#         )
...
#             os.makedirs(self.offl_folder, exist_ok=True)
#             os.makedirs(self.data_folder, exist_ok=True)
#         except Exception as e:
#             st.error(f"Error creating directories: {e}")  # Display error if folder creation fails

#     def load_docx(self, file):
#         """Load content from a DOCX file."""
#         try:
#             # Read all paragraphs from a DOCX file and join them into a single string
#             doc = Document(file)
#             return "\n".join(paragraph.text for paragraph in doc.paragraphs)
#         except Exception as e:
#             st.error(f"Error loading DOCX file: {e}")  # Handle errors in reading DOCX file
#             return ""
...
#                 text.append(page.extract_text())  # Append text from each page
#             return "\n".join(text)  # Return the full PDF content as a single string
#         except Exception as e:
#             st.error(f"Error processing PDF file: {e}")  # Handle errors in reading PDF file
#             return ""

#     def file_preprocessing(self, file):
#         """Preprocess the uploaded file to extract text."""
#         try:
#             # Determine file type by its extension, and process accordingly
#             if file.name.endswith('.pdf'):
#                 return self.load_pdf(file)  # Extract text from PDF
#             elif file.name.endswith('.docx'):
#                 return self.load_docx(file)  # Load content from DOCX
#             else:
#                 raise ValueError("Unsupported file format")  # Raise an error if format is not supported
#         except Exception as e:
...
#         try:
#             # Create the summarization pipeline using the pre-trained model and tokenizer
#             pipe_sum = pipeline(
#                 'summarization',
#                 model=self.base_model,
#                 tokenizer=self.tokenizer,
#                 max_length=500,  # Set max length of the summary
#                 min_length=50  # Set minimum length of the summary
#             )
#             # Extract text from the file
#             input_text = self.file_preprocessing(file)
            
#             if input_text is None:  # Stop the process if text extraction failed
#                 return None
            
#             # Generate summary and return the first result (summary text)
#             result = pipe_sum(input_text)
#             return result[0]['summary_text']
#         except Exception as e:
#             st.error(f"Error generating summary: {e}")  # Handle errors in the summarization process
#             return None  # Return None to indicate failure
...
#         try:
#             # Read the file as bytes and convert to base64 format for embedding in HTML
#             pdf_bytes = file.read()
#             base64_pdf = base64.b64encode(pdf_bytes).decode('utf-8')
#             # Create an iframe to display the PDF
#             pdf_display = f'<iframe src="data:application/pdf;base64,{base64_pdf}" width="100%" height="600" type="application/pdf"></iframe>'
#             st.markdown(pdf_display, unsafe_allow_html=True)  # Display PDF in the Streamlit app
#         except Exception as e:
#             st.error(f"Error displaying PDF: {e}")  # Handle errors in displaying the PDF

#     def display_docx(self, file):
#         """Display the content of a DOCX file in the Streamlit app."""
#         try:
#             # Extract DOCX content and display it in a text area
#             doc_content = self.load_docx(file)
#             st.text_area("Document Content", value=doc_content, height=400)
#         except Exception as e:
#             st.error(f"Error displaying DOCX content: {e}")  # Handle errors in displaying DOCX content
...
#                 # Split the layout into two columns for file preview and summary output
...
#                         self.display_pdf(uploaded_file)  # Display PDF preview
#                     elif uploaded_file.name.endswith('.docx'):
#                         self.display_docx(uploaded_file)  # Display DOCX content
...
#                         unsafe_allow_html=True
#                     )
#                     summary = self.llm_pipeline(uploaded_file)  # Generate summary
                    
#                     if summary is None:
#                         st.warning("⚠️ Failed to generate summary. Please try again.")  # Handle summarization failure
#                     else:
...
#     try:
#         # Attempt to remove the offload folder after use (cleanup)
#         os.removedirs(name="offload_dir")
#     except:
#         pass
